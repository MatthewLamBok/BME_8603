...
# Number inputs with session state persistence
length = st.number_input('Length', min_value=0.0, max_value=1.0, value=st.session_state.get('length', 0.5), step=0.01)
diameter = st.number_input('Diameter', min_value=0.0, max_value=1.0, value=st.session_state.get('diameter', 0.4), step=0.01)
height = st.number_input('Height', min_value=0.0, max_value=1.0, value=st.session_state.get('height', 0.1), step=0.01)
whole_weight = st.number_input('Whole Weight', min_value=0.0, max_value=3.0, value=st.session_state.get('whole_weight', 1.5), step=0.01)
shucked_weight = st.number_input('Shucked Weight', min_value=0.0, max_value=2.0, value=st.session_state.get('shucked_weight', 0.6), step=0.01)
viscera_weight = st.number_input('Viscera Weight', min_value=0.0, max_value=1.0, value=st.session_state.get('viscera_weight', 0.3), step=0.01)
shell_weight = st.number_input('Shell Weight', min_value=0.0, max_value=1.0, value=st.session_state.get('shell_weight', 0.4), step=0.01)
